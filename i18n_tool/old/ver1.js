...
// const studioPath = '/home/xyz/xyz-studio-front/src/app/test/';
...
function regFile(filePath) {
  ...
  if (ext === ".ts") {
    reg = new RegExp(
      "(translate|translateAsync|instant)\\(([\\s\\S]*?)\\)",
      "g"
    );
    let newKeyWords = [];
    while ((result = reg.exec(data))) {
      const keyWords = RegExp.$2.replaceAll("'", '"');
      const regKeyWords = new RegExp('\\"(\\w+?.\\w+?)\\"', 'g')
      while (r = regKeyWords.exec(keyWords)) {
          newKeyWords.push(RegExp.$1)
      }
    }
    ...
    let reg2 = new RegExp(`(${newKeyWords.join("|")})`, "g");
